import os
import sys
sys.path.insert(0, '/Users/dennisonbertram/Develop/rl-irs-tax-code/.venv/lib/python3.14/site-packages')

from huggingface_hub import HfApi, login

login(token=os.environ["HF_TOKEN"])  # set HF_TOKEN env var or run `huggingface-cli login`
api = HfApi()

USERNAME = "dennisonb"

# 1. Upload GGUF repo
REPO = f"{USERNAME}/qwen25-tax-3b-GGUF"
print(f"Creating repo {REPO}...")
api.create_repo(repo_id=REPO, repo_type="model", exist_ok=True)

print("Uploading model card...")
api.upload_file(path_or_fileobj="/tmp/model_card.md", path_in_repo="README.md", repo_id=REPO)

print("Uploading GGUF (3.1 GB, this will take a few minutes)...")
api.upload_file(path_or_fileobj="/tmp/qwen25-tax-3b-q8_0.gguf", path_in_repo="qwen25-tax-3b-q8_0.gguf", repo_id=REPO)

print(f"GGUF uploaded: https://huggingface.co/{REPO}")

# 2. Upload adapters repo
ADAPTER_REPO = f"{USERNAME}/qwen25-tax-3b-adapters"
print(f"\nCreating repo {ADAPTER_REPO}...")
api.create_repo(repo_id=ADAPTER_REPO, repo_type="model", exist_ok=True)

for stage in ["sft", "dpo", "grpo"]:
    print(f"Uploading {stage} adapters...")
    api.upload_folder(
        folder_path=f"/Users/dennisonbertram/Develop/rl-irs-tax-code/outputs/{stage}/adapters",
        path_in_repo=f"{stage}/",
        repo_id=ADAPTER_REPO,
    )

print(f"Adapters uploaded: https://huggingface.co/{ADAPTER_REPO}")
print("\nDone!")
