# Training Pipeline Review Package v5

## Environment
MLX 0.31.1, Python 3.14, Apple Silicon
Model: Qwen2.5-3B-Instruct. Pipeline: SFT -> DPO -> GRPO RL

## Round 4 -> Round 5 Fixes Applied

### CRITICAL resolved: _get_model_items tuple/treedef guard
Added explicit check: if result is a 2-tuple where [1] is a type (PyTreeDef-style),
extract result[0] as the leaves list. Added type validation for each element.
Both train_grpo.py and train_dpo.py updated with identical defensive code.

### MEDIUM resolved: GRPO value_and_grad compiled once outside loop
loss_fn now takes (prompt, completions, rewards) as explicit args instead of a
per-step closure. loss_and_grad = nn.value_and_grad(policy_model, loss_fn) is
defined once before the training loop. Called as loss_and_grad(prompt, completions, rewards).

### MEDIUM resolved: GRPO gradient clipping added
Added optim.clip_grad_norm(grads, max_norm=1.0) to match DPO training loop.

### LOW resolved: Modelfile updated to reference model-q6_k.gguf
Was referencing stale model-q8.gguf path.

### NOTE on SFT adapter_config.json num_layers=16
This is correct — it reflects an ACTUAL training run with 16 layers.
The script default was changed to 24 for FUTURE runs, but the file is
an accurate record of what was trained. Not a bug.
