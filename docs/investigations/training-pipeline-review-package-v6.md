# Training Pipeline Review Package v6 (Round 5)

## Environment
MLX 0.31.1, Python 3.14, Apple Silicon M4 Max
Model: Qwen2.5-3B-Instruct. Pipeline: SFT -> DPO -> GRPO RL

## VERIFIED MLX BEHAVIOR (empirically tested):
  mlx.utils.tree_flatten(model.parameters()) returns list[tuple[str, array]]
  NOT tuple(leaves, treedef). This has been empirically verified in MLX 0.31.1.
  The _get_model_items() function handles both cases defensively.

## VERIFIED value_and_grad calling convention (empirically tested):
  nn.value_and_grad(model, fn) returns wrapped_fn where:
    wrapped_fn(*args) calls fn(*args) — model is NOT prepended to args.
  The inner_fn signature is inner_fn(params, *args) where params = trainable params.
  fn receives exactly the args passed to wrapped_fn, nothing more.

  GRPO uses: loss_fn(prompt, completions, rewards) — called as loss_and_grad(prompt, completions, rewards)
  DPO uses: loss_fn(policy_model, batch) — called as loss_and_grad(policy_model, batch)
  Both patterns are correct.

## Round 5 -> Round 6 Fixes Applied

### GRPO LoRA re-apply guard added (_apply_lora_if_needed)
Prevents duplicate LoRA initialization on resume runs.

### mx.metal.clear_cache() guarded in evaluate.py
Wrapped in hasattr check for MLX >= 0.30 compatibility.

### train_sft.py build_lora_config() unused imports removed
...
def train(args: argparse.Namespace, model_path: Path) -> None:
    ...
    def _apply_lora_if_needed(mdl, num_layers, config):
        """Apply LoRA only if layers don't already have lora_A (fix 5-B / resume safety).
        Re-applying LoRA to a model that already has LoRA layers would raise duplicate
        parameter errors or silently double the parameter count.
        """
        first_layer = next(iter(mdl.model.layers), None) if hasattr(mdl, "model") else None
        if first_layer is not None and hasattr(first_layer, "lora_A"):
...
```
FROM /Users/dennisonbertram/Develop/rl-irs-tax-code/outputs/final/model-q6_k.gguf
