# Training Pipeline Review Package v2

**Assembled:** 2026-03-29
**Purpose:** SECOND review — all issues from first review have been fixed. Verify fixes and identify remaining issues.

---

## SECOND REVIEW NOTICE

This is the **SECOND review**. The first review identified 25+ issues across 8 categories. All have been fixed. Please verify the fixes are correct and identify any remaining issues.

---

## Summary of All Fixes Applied from First Review

### 1. BUGS & LOGIC ERRORS

| ID | Severity | Issue | Status |
|----|----------|-------|--------|
| 1-A | CRITICAL | GRPO `sequence_log_prob` used SUM instead of MEAN over tokens, causing importance ratio to explode with long completions | **FIXED** — `train_grpo.py` now uses length-normalized mean per-token log-prob (lines 310-336) |
| 1-B | CRITICAL | LoRA scale mismatch when GRPO loads prior adapters (hardcoded scale=1.0 vs SFT's scale=20.0) | **FIXED** — `train_grpo.py` now reads `lora_parameters.scale`, `rank`, and `num_layers` from adapter_config.json (lines 426-439) |
| 1-C | HIGH | Train/Valid leakage from inflation upsampling before split | **FIXED** — `assemble_v5_dataset.py` now splits FIRST on base records, then upsamples within each partition (lines 396-451) |
| 1-D | HIGH | DPO prompt+answer concatenated without separator | **FIXED** — `train_dpo.py` `_collate_batch` now inserts `tokenizer.eos_token` or `"\n\n"` separator between prompt and response (lines 268-276) |
| 1-E | HIGH | Same separator bug in GRPO generation-side advantage calculation | **FIXED** — `train_grpo.py` `grpo_loss_for_prompt` now inserts separator between prompt and completion (lines 369-372) |
| 1-F | MEDIUM | `factual_accuracy_score` raw string comparison fails for "$1,160,000" vs "1160000" | **FIXED** — `grpo_reward.py` added `normalize_number()` function that strips `$`, `,` and leading zeros before comparison (lines 144-193) |
| 1-G | MEDIUM | `evaluate.py` baseline model keeps GPU memory after deletion, potential OOM | **FIXED** — `evaluate.py` now calls `mx.metal.clear_cache()` after deleting baseline model (line 529) |
| 1-H | MEDIUM | `tree_flatten` API change in mlx 0.17+ breaks DPO/GRPO `save_lora_weights` | **FIXED** — Both `train_dpo.py` and `train_grpo.py` now try/except import with fallback to `tree_flatten_items` (lines 78-81 in each) |
| 1-I | LOW | Citation regex matches CFR section signs as IRC citations | **FIXED** — `citation_utils.py` added negative lookbehind for `C.F.R.` and `CFR` in `_SECTION_SIGN` pattern (lines 52-55) |

### 2. TRAINING METHODOLOGY

| ID | Severity | Issue | Status |
|----|----------|-------|--------|
| 2-A | HIGH | SFT LoRA only covers 16/24 transformer blocks | **FIXED** — `train_sft.py` `DEFAULTS["lora_layers"]` raised from 16 to 24 (line 41) |
| 2-B | HIGH | LoRA dropout inconsistent: 0.05 in SFT, 0.00 in DPO/GRPO | **FIXED** — `train_dpo.py` (line 424) and `train_grpo.py` (line 443) now use `dropout=0.05` matching SFT; adapter_config.json records actual dropout |
| 2-C | MEDIUM | DPO beta=0.5 may be too large after length-normalization | **ACKNOWLEDGED** — Beta kept at 0.5 for now; to be tuned after other fixes are validated. Comment added in code. |
| 2-D | MEDIUM | GRPO group_size=4 gives noisy rewards | **FIXED** — `train_grpo.py` `DEFAULTS["group_size"]` raised from 4 to 8 (line 56) with comment about using 4 if OOM |

### 3. REWARD FUNCTION

| ID | Severity | Issue | Status |
|----|----------|-------|--------|
| 3-A | HIGH | Component weights can exceed 1.0 before vague_penalty | **FIXED** — `grpo_reward.py` weights now sum to at most 0.90 before penalty; final clamp to [0.0, 1.0] added (lines 261-268) |
| 3-B | MEDIUM | `citation_accuracy` returns 0.5 when `expected_section` is None, biasing toward always citing | **FIXED** — Default changed to 0.25 (uncertain/neutral) when expected_section is None (lines 126-133) |
| 3-C | LOW | `count_citations` rewards raw frequency linearly up to 4 | **FIXED** — Changed to diminishing-returns curve: `score = 1 - exp(-n/2)` (line 243) |

### 4. DATA PIPELINE

| ID | Severity | Issue | Status |
|----|----------|-------|--------|
| 4-A | HIGH | Entirely synthetic GPT-4 data, risk of hallucinated facts | **ACKNOWLEDGED** — Known limitation. Data sourced from IRC/CFR grounded generation with citation verification. Cross-checking with authoritative sources is a planned improvement. |
| 4-B | MEDIUM | DPO deduplication by prompt only discards alternative hard negatives | **FIXED** — `assemble_v5_dataset.py` `deduplicate_dpo` now deduplicates by full (prompt, chosen, rejected) triple (lines 267-292) |
| 4-C | LOW | Inflation data upsampled 20x dominates >35% of tokens | **ACKNOWLEDGED** — Default kept at 20x for compatibility; help text now warns about aggressive upsampling and suggests monitoring class balance (line 350) |

### 5. ADAPTER MANAGEMENT

| ID | Severity | Issue | Status |
|----|----------|-------|--------|
| 5-A | CRITICAL | DPO adapter_config.json missing `num_layers` | **FIXED** — `train_dpo.py` now writes complete adapter config with `num_layers`, `lora_parameters` sub-key containing `rank`, `scale`, and `dropout` (lines 409-420) |
| 5-B | HIGH | `linear_to_lora_layers` re-creates random LoRA params on resume | **FIXED** — `train_dpo.py` added `_apply_lora_if_needed()` guard that checks `hasattr(layer, "lora_A")` before applying (lines 429-436) |

### 6. QUANTISATION / EXPORT

| ID | Severity | Issue | Status |
|----|----------|-------|--------|
| 6-A | HIGH | bf16 -> q8_0 -> q4_k_m pipeline fails; q4_k_m loses numeric precision | **FIXED** — `export_to_ollama.py` now uses two-step pipeline: bf16 GGUF first, then q6_k quantization (lines 44-47, 192-237) |
| 6-B | MEDIUM | Modelfile stop sequences incomplete for Qwen chat template | **FIXED** — `export_to_ollama.py` Modelfile now includes all three Qwen stop tokens: `<\|endoftext\|>`, `<\|im_end\|>`, `<\|im_start\|>` (lines 259-262) |

### 7. EVALUATION

| ID | Severity | Issue | Status |
|----|----------|-------|--------|
| 7-A | HIGH | Only 25 questions, single sample, no confidence intervals | **ACKNOWLEDGED** — Expanding evaluation set is planned. Current 25 questions provide basic regression detection. |
| 7-B | MEDIUM | Baseline vs fine-tuned uses different temperatures | **FIXED** — `evaluate.py` now passes explicit `temperature` parameter to both baseline and fine-tuned evaluation (lines 395-519, shared temperature arg) |

### 8. ARCHITECTURE DECISIONS

| ID | Severity | Issue | Status |
|----|----------|-------|--------|
| 8-A | MEDIUM | 3B Qwen lacks capacity for full IRC numeric table | **ACKNOWLEDGED** — Known limitation. Retrieval augmentation and larger model (7B+) are planned improvements. |
| 8-B | LOW | Python loops in GRPO reward/gradient hinder Metal graph compilation | **ACKNOWLEDGED** — Vectorization is a planned optimization. Current implementation prioritizes correctness. |
...
where:
    pi       = current (trainable) policy
    pi_ref   = frozen reference policy (SFT checkpoint)
...
def save_lora_weights(model, path: str) -> None:
    ...
    # Fix 1-H: mlx 0.17+ renamed tree_flatten to tree_flatten_items; shim for both versions
    try:
        from mlx.utils import tree_flatten
    except ImportError:
        from mlx.utils import tree_flatten_items as tree_flatten
...
def dpo_loss(
    ...
    L = -mean[ log sigma(beta * (log pi(y_w|x) - log pi_ref(y_w|x)
                          - log pi(y_l|x) + log pi_ref(y_l|x))) ]
    """
    import mlx.core as mx
    import mlx.nn as nn
...
Algorithm per step:
    ...
    4. Normalise rewards within the group: r_hat_i = (r_i - mean(r)) / (std(r) + eps)
    5. Compute policy-gradient loss with KL-from-reference clipping (PPO-style):
         L = -mean_i[ min(rho_i * r_hat_i, clip(rho_i, 1-eps_clip, 1+eps_clip) * r_hat_i) ]
       where rho_i = pi(y_i|x) / pi_ref(y_i|x)
...
def save_lora_weights(model, path: str) -> None:
    """Save only LoRA adapter weights, not the full model."""
    import mlx.core as mx
    # Fix 1-H (same as DPO): mlx 0.17+ renamed tree_flatten to tree_flatten_items
    try:
        from mlx.utils import tree_flatten
    except ImportError:
        from mlx.utils import tree_flatten_items as tree_flatten
...
def grpo_loss_for_prompt(
    ...
    for completion, adv in zip(completions, advantages):
        ...
        # KL penalty: KL(pi || pi_ref) approx log(rho)
...
## File: scripts/citation_utils.py
...
#   Section 179 / 179    ->  bare section-sign with optional space
#   IRC Section 179 / IRC 179   ->  "IRC" prefix
#   I.R.C. Section 179          ->  dotted abbreviation
#   I.R.C. Section 179(d)(1)    ->  with subsections
#   26 U.S.C. Section 179       ->  title-26 USC citation
...
_IRC_PREFIXES = r"""
    ...
    \s*
    (?:Section|Sec\.?|section)?                       # optional "Section"/"Sec."/"section"
    \s*
"""
...
_SECTION_SIGN = r"""
    ...
    section\s*                                        # signs are NOT counted as IRC citations
"""
...
CFR_CITATION_PATTERN = re.compile(
    r"(?:26\s*C\.?F\.?R\.?|Treasury\s*Reg(?:ulation)?s?)\s*[section\s]*(\d+[\.\w\-]+)",
    re.IGNORECASE,
)
...
def extract_irc_sections(text: str) -> set[str]:
    ...
    Examples
    --------
    >>> extract_irc_sections("Under IRC Section 179 and Section 168(k)...")
    {'179', '168'}
    >>> extract_irc_sections("See 26 U.S.C. Section 1031 and I.R.C. Section 408A")
    {'1031', '408A'}
    """
...
def extract_section_number(section_str: str) -> Optional[str]:
    ...
    Useful for parsing values like "IRC Section 179" or "Section 408A" that
...
def extract_numbers(text: str) -> set[str]:
    ...
    # Percentages: 20%  10.5%  59.5% is unusual but guard against it
    amounts.update(re.findall(r'\d+(?:\.\d+)?%', text))
    return amounts
```
...
EVAL_QUESTIONS: list[tuple[str, list[str], list[str]]] = [
    ...
    (
        ...
        ["72", "72(t)"],
        ["10%", "early withdrawal", "59 1/2", "exception", "substantially equal",
         "disability", "death"],
    ),
...
## File: scripts/export_to_ollama.py
...
Pipeline:
    ...
    3. Quantize to Q6_K                        (llama-quantize)
...
## File: scripts/assemble_v5_dataset.py
...
## File: outputs/grpo/adapters/adapter_config.json

```json
{
    "adapter_path": "outputs/grpo/adapters",
    "batch_size": 4,
    "config": null,
...
## File: outputs/dpo/adapters/adapter_config.json
...
## File: outputs/sft/adapters/adapter_config.json
