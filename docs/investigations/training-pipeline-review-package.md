# Training Pipeline Review Package

**Assembled:** 2026-03-29
**Purpose:** Complete code + config snapshot for GPT-5.4 architecture review

---

## Summary

### v5 Dataset Stats (`wc -l data/v5/*.jsonl`)

| File | Lines |
|------|-------|
| `dpo_train.jsonl` | 23,309 |
| `dpo_valid.jsonl` | 2,590 |
| `grpo_train.jsonl` | 99,504 |
| `grpo_valid.jsonl` | 11,057 |
| `sft_train.jsonl` | 99,504 |
| `sft_valid.jsonl` | 11,057 |
| `train.jsonl` (copy of sft_train) | 99,504 |
| `valid.jsonl` (copy of sft_valid) | 11,057 |
| **Total** | **357,582** |

### Current Adapter File Sizes and Dates

**SFT adapters** (`outputs/sft/adapters/`):
- `adapter_config.json` -- 1.1K, Mar 29 16:37
- `adapters.safetensors` -- 102M, Mar 29 17:30 (final, step 1500)
- 7 intermediate checkpoints (steps 200-1400) -- 102M each, Mar 29 16:40-17:25

**DPO adapters** (`outputs/dpo/adapters/`):
- `adapter_config.json` -- 112B, Mar 29 21:27
- `adapters.safetensors` -- 102M, Mar 30 01:51
- `adapters_best.safetensors` -- 102M, Mar 30 00:55

**GRPO adapters** (`outputs/grpo/adapters/`):
- `adapter_config.json` -- 988B, Mar 26 03:50
- `adapters.safetensors` -- 102M, Mar 30 20:43
- `adapters_best.safetensors` -- 102M, Mar 30 15:48

### Known Bugs Found So Far

1. **GRPO init order bug** (`train_grpo.py`): `load_weights()` was called BEFORE `linear_to_lora_layers()`, so all DPO adapter weights were silently discarded (`strict=False`). GRPO trained from fresh base model, not from DPO warm-start. **FIX APPLIED** -- order swapped so LoRA layers exist before weights are loaded.

2. **DPO length normalization bug** (`train_dpo.py`): Original `sequence_log_prob()` used SUM over tokens instead of MEAN, causing log-ratios to scale with sequence length. Long sequences produced huge magnitudes that saturated the sigmoid regardless of beta, collapsing all losses to {0.3133, 0.8133, 1.3133}. **FIX APPLIED** -- now uses per-token average (length normalization).

3. **GGUF quantization precision loss**: Q8_0 quantization degrades fine-tuned numeric accuracy in a 3B model. SFT adapter gives correct $14,600 standard deduction on MLX but hallucinates $15,000 after Q8_0 GGUF conversion. GRPO GGUF shifts from $12,950 (base model figures) to $12,550 (2021 figures). BF16 GGUF recommended instead.

4. **DPO adapter_config.json missing `num_layers` key**: Causes crash when trying to load with `mlx_lm.load`. GRPO adapter config uses `num_layers` inconsistently vs `lora_layers`.

### v5 Eval Results Summary

- **Eval timestamp**: 2026-03-29 00:10:43
- **Adapter evaluated**: `outputs/grpo/adapters` (NOTE: this was the broken GRPO adapter -- trained from base, not DPO)
- **25 questions**, max_tokens=512, temperature=0.3
- **Mean overall score**: 0.7489 / 1.000
- **Mean citation score**: 0.384 / 0.400 (96% of questions had correct IRC section cited)
- **Mean keyword score**: 0.2529 / 0.400
- **No baseline comparison** was saved in this eval run (finetuned-only mode)
...
def _collate_batch(
    ...
    chosen_seqs = [
        encode(_extract_dpo_text(r, "prompt") + _extract_dpo_text(r, "chosen")) for r in records
    ]
    rejected_seqs = [
        encode(_extract_dpo_text(r, "prompt") + _extract_dpo_text(r, "rejected")) for r in records
    ]
...
def train(args: argparse.Namespace, model_path: Path) -> None:
    ...
    linear_to_lora_layers(ref_model, args.lora_layers, {"rank": args.lora_rank, "scale": 1.0, "dropout": 0.0})
    if start_adapter is not None:
...
def deduplicate_dpo(records: list[tuple]) -> tuple[list[tuple], dict]:
    """Deduplicate DPO records by prompt text. Keep first occurrence."""
    seen = set()
    deduped = []
    dup_count = 0

    for record, source, is_inflation in records:
        key = _dpo_prompt_key(record.get("prompt", ""))
        if key in seen:
            dup_count += 1
            continue
        seen.add(key)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Inflation upsampling multiplier (default: 20)",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    print("--- SFT Inflation Upsampling ---")
    inflation_sft_base = sum(
        1 for _, _, is_inf in sft_deduped if is_inf
    )
    sft_upsampled, sft_extra = apply_inflation_upsampling(
        sft_deduped, args.inflation_multiplier
    )
    print(
        f"  Inflation records (base): {inflation_sft_base:,}  "
        f"Extra copies added: {sft_extra:,}  "
        f"Total SFT after upsampling: {len(sft_upsampled):,}"
    )
    print()

    print("--- SFT Train/Valid Split ---")
    sft_train_tuples, sft_valid_tuples = train_valid_split(
        sft_upsampled, args.split_ratio, args.seed
    )
    print(
        f"  Train: {len(sft_train_tuples):,}  "
        f"Valid: {len(sft_valid_tuples):,}  "
        f"Ratio: {len(sft_train_tuples)/len(sft_upsampled):.3f}"
    )
    print()
    ...
    print("--- DPO Inflation Upsampling ---")
    inflation_dpo_base = sum(1 for _, _, is_inf in dpo_deduped if is_inf)
    dpo_upsampled, dpo_extra = apply_inflation_upsampling(
        dpo_deduped, args.inflation_multiplier
    )
    print(
        f"  Inflation records (base): {inflation_dpo_base:,}  "
        f"Extra copies added: {dpo_extra:,}  "
        f"Total DPO after upsampling: {len(dpo_upsampled):,}"
    )
    print()

    print("--- DPO Train/Valid Split ---")
    dpo_train_tuples, dpo_valid_tuples = train_valid_split(
        dpo_upsampled, args.split_ratio, args.seed
    )
    print(
        f"  Train: {len(dpo_train_tuples):,}  "
        f"Valid: {len(dpo_valid_tuples):,}  "
        f"Ratio: {len(dpo_train_tuples)/len(dpo_upsampled):.3f}"
    )
    print()
    ...
    report["sft"] = {
        ...
        "total_after_upsampling": len(sft_upsampled),
        "train_count": len(sft_train),
        "valid_count": len(sft_valid),
        "actual_split_ratio": len(sft_train) / len(sft_upsampled),
    }
    report["dpo"] = {
        ...
        "total_after_upsampling": len(dpo_upsampled),
        "train_count": len(dpo_train),
        "valid_count": len(dpo_valid),
        "actual_split_ratio": len(dpo_train) / len(dpo_upsampled),
    }
    report["grpo"] = {
    ...
    print("=" * 70)
    print(f"  SFT total (after dedup + upsample) : {len(sft_upsampled):,}")
    print(f"    train  : {len(sft_train):,}")
    print(f"    valid  : {len(sft_valid):,}")
    print(f"  DPO total (after dedup + upsample) : {len(dpo_upsampled):,}")
...
## docs/investigations/ollama-vs-mlx-debug-2026-03-31.md
...
## docs/investigations/training-data-quality-audit.md (first 50 lines)
...
[... truncated at line 50 -- see full file for remaining audit details ...]
```
