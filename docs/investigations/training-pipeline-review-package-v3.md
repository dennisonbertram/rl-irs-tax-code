# Training Pipeline Review Package v3

## Context
This is a tax-law LLM fine-tuning pipeline using MLX on Apple Silicon.
Model: Qwen2.5-3B-Instruct with LoRA adapters.
Pipeline: SFT -> DPO -> GRPO RL

## Fixes Applied in This Round (from v2 review)

### CRITICAL fix: GRPO value_and_grad invocation
Changed loss_fn from taking a `model` argument to a zero-argument closure.
Before: `nn.value_and_grad(policy_model, loss_fn)(policy_model)` where `loss_fn(model)` took model as arg
After: `nn.value_and_grad(policy_model, loss_fn)()` where `loss_fn()` is a closure over `policy_model`

### HIGH fix: save_lora_weights tree_flatten guard
Reversed try/except order: now tries `tree_flatten_items` first (mlx >=0.17),
falls back to `tree_flatten` for older releases. Both return (key, value) tuples.

### HIGH fix: GRPO adapter_config.json not written
Added code to write adapter_config.json at every checkpoint save in train_grpo.py.
Uses nested lora_parameters format: {rank, scale, dropout} consistent with DPO/SFT.

### LOW fix: DPO deterministic epoch shuffles
batch_iterator now accepts an `epoch` parameter to vary the RNG seed per epoch.
Training loop increments epoch counter on each StopIteration.

### LOW fix: Evaluation false negatives for subsection citations
score_response now strips subsection qualifiers (e.g. '1(h)' -> '1') before
comparing to extract_irc_sections() output which only returns base numbers.

### LOW fix: SFT --lora-rank flag wired up
build_lora_config() generates a YAML config with the requested rank.
Previously the flag was accepted but the static rank32.yaml was always used.

### LOW fix: SFT DATA_DIR path matches v5 directory structure
Changed default DATA_DIR from data/processed/train to data/v5.
...
def save_lora_weights(model, path: str) -> None:
    ...
    # Fix HIGH (tree_flatten guard): try tree_flatten_items first (present on
    # mlx >=0.17 and behaves like old tree_flatten).  Fall back to tree_flatten
    # for older releases.  Both return a list of (key, value) tuples suitable
    # for dict comprehension.
    try:
        from mlx.utils import tree_flatten_items as _flatten
    except ImportError:
        from mlx.utils import tree_flatten as _flatten
    all_params = _flatten(model.parameters())
    lora_params = {k: v for k, v in all_params if "lora" in k}
    if not lora_params:
        print("WARNING: No LoRA parameters found, saving all trainable params")
        lora_params = dict(_flatten(model.trainable_parameters()))
